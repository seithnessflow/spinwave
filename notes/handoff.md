...
State at 2026-09-12. 564 tests green, clippy silent, working tree clean.
...
- **70 cases, 58 match, 12 tracked** in `KNOWN_DIVERGENCES`.
...
1. **The `mod_*` cases — mostly SOLVED, and it was the bench.**
   ...
   **The level cases were then two engine bugs, found with the destination
   ...
   Still failing: `mod_env_to_pitch` (2.6e-1, undiagnosed — next candidate
   for the destination probe on the oscillator transpose),
   `mod_random_to_cutoff` (6.7e-2), and the two near misses.
   ...
   **Five diagnoses died getting here** — an onset ramp, the poly route,
   ...
   Still open: `--probe` found Spinwave one control block (2.9 ms)
   **ahead** of the reference on every source, structural. Two fixes were
   tried and reverted: resolving the matrix before `update_modulators`
   broke the note/velocity cases (57 → 55), and a surgical "read before
   advance" moved the env cases by a few percent. Neither is the cause of
   any tracked case except, plausibly, the `mod_lfo_to_level` near miss.
