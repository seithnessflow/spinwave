...
- **64 cases, 52 match, 12 tracked** in `KNOWN_DIVERGENCES`.
...
1. **The five `mod_*` cases** — the highest-value item by far. Read them
   in the relative column: **-4 to +1 dB**, meaning the error is as loud as
   the render. These are not near misses.

   An earlier version of this file called it an onset ramp and blamed
   smoothing. **That was wrong.** Two experiments say what it is:

   * `mod_macro_to_cutoff` MATCHES (-58 dB rel). Its source never moves, so
     the destination path, the transform, the amount and the destination
     scale are all exonerated on the same route the failing cases take.
     What separates it: a macro is **mono**, so
     `SoundEngine::connectModulation` routes it to the mono destination.
     Every failing case has a **poly** source (LFO, envelope, random),
     which takes `change.poly_destination` and a different path. Suspect
     that path — in particular the double fold across voice lanes, once in
     `VoiceHandler::writeNonaccumulatedOutputs` and again in
     `SynthVoiceHandler::process` (`masked + swapVoices(masked)`). Getting
     that wrong scales the depth rather than shifting it.
   * `--probe` compared the source curves block by block: they agree to
     3e-4, so the sources themselves are right. It did find Spinwave
     running exactly one control block (2.9 ms) **ahead** of the reference
     on every source, persisting on a note that starts precisely at a block
     boundary, so it is structural rather than an intra-block offset
     artefact. Real and worth fixing, but **not this**: consuming the
     previous block's sources moves these numbers by a few percent.

   The bipolar case passing proves less than it looks — a bipolar source
   has a near-zero mean, so an error acting on the slow component of the
   modulation shows up far more weakly there.
...
**One trap, and it cost an hour.** On the reference side, read the
