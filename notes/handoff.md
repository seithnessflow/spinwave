...
1. **The `mod_*` cases — mostly SOLVED, and it was the bench.**
   ...
   bypass, in `ModulationConnectionProcessor::init()`), and the harness had
   already captured its control map by then. Those controls never got
   their table defaults and kept what they were constructed with —
   **bipolar**. Every modulation reference in the corpus was rendered
   bipolar however its case was written.
