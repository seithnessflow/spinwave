...
State at 2026-09-12. 593 tests green, clippy silent, working tree clean.
...
- **83 cases, 70 match, 13 tracked** in `KNOWN_DIVERGENCES`, at the tightened bounds. The gap the bounds sit in was re-checked after the last wave of fixes: still nothing between 1e-5 and 1e-4.
...
## The 13 tracked divergences (at RMS 1e-4 / peak 2e-3)
...
- **The operations refuse 62 of those 75** because they modulate
  destinations Spinwave does not route: 356 connections, 174 of them into
  `modulation_N_amount` (a macro or the velocity on another connection's
  amount — Vital's meta-modulation), then `lfo_N_tempo` (26),
  `osc_N_detune_range` (15), `filter_fx_mix` (14), `random_N_tempo` (11),
  `flanger_center` (7), and some thirty more. Not reading: the voice
  matrix and the effects matrix lack the destinations. Scoped in the
  operations design note's last section; it is what blocks the
  ten-sounds test's condition A on real material.
