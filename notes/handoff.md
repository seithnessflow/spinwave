...
State at 2026-09-12. 591 tests green, clippy silent, working tree clean.
...
- **83 cases, 69 match, 14 tracked** in `KNOWN_DIVERGENCES`, at the tightened bounds.
...
## The 14 tracked divergences (at RMS 1e-4 / peak 2e-3)
...
`osc_unison` 3.2e-4 and `fx_chorus` 1.1e-4 (undiagnosed), and
`mod_lfo_to_distortion_drive` 9.1e-4 (the rate). The phaser's three
cases, which would have been promoted too, went to float noise once the
coefficient came from the reference's lookup table instead of `tan()`.
...
- **The sub oscillator is not implemented.** The reference's `sub_*`
  controls (eight of them) are dropped by every preset that uses it.
  Found by the read-parameter audit (`patch::read_audit`), which lists
  it, `osc_N_smooth_interpolation` and the keytracked LFO rates as
  FINDINGS with reasons — a name leaves that list when the control is
  wired. (The formant filter's five controls and the LFOs' `sync_type`
  were on it for a day and are wired now, with golden cases.)
- The plugin ignores the preset's `oversampling` (the offline session
  applies it); the plugin follows its host and never sets it.
