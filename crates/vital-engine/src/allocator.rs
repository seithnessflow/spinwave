...
use vital_poly::constants::{VoiceEvent, NOTES_PER_OCTAVE, NUM_MIDI_CHANNELS};
...
impl<K: VoiceKernel> VoiceAllocator<K> {
    ...
    /// the caller sums into its mix bus (remember lanes hold two voices —
...
mod tests {
    ...
    fn polyphony_limit_steals_voices() {
        ...
        // voice fades via kill event — active count includes the dying one.
